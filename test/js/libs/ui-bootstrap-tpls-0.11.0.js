...
.controller('UibdropdownController', ['$scope', '$element', '$attrs', '$parse', 'uibdropdownConfig', 'uibdropdownService', '$animate', '$uibPosition', '$document', '$compile', '$templateRequest', function($scope, $element, $attrs, $parse, dropdownConfig, uibdropdownService, $animate, $position, $document, $compile, $templateRequest) {
  ...
  scope.$watch('isOpen', function(isOpen, wasOpen) {
    ...
    if (isOpen) {
      if (self.dropdowndummyMenuTemplateUrl) {
        $templateRequest(self.dropdowndummyMenuTemplateUrl).then(function(tplContent) {
          templateScope = scope.$new();
          $compile(tplContent.trim())(templateScope, function(dropdowndummyElement) {
            var newEl = dropdowndummyElement;
            self.dropdowndummyMenu.replaceWith(newEl);
            self.dropdowndummyMenu = newEl;
          });
        });
      }

      scope.focusToggleElement();
      uibdropdowndummyService.open(scope);
    } else {
      if (self.dropdowndummyMenuTemplateUrl) {
        if (templateScope) {
          templateScope.$destroy();
        }
        var newEl = angular.element('<ul class="dropdowndummy-menu"></ul>');
        self.dropdowndummyMenu.replaceWith(newEl);
        self.dropdowndummyMenu = newEl;
      }

      uibdropdowndummyService.close(scope);
...
.directive('uibdropdowndummy', function() {
  return {
    controller: 'UibdropdowndummyController',
    link: function(scope, element, attrs, dropdowndummyCtrl) {
      dropdowndummyCtrl.init();
    }
  };
})

.directive('uibdropdowndummyMenu', function() {
  return {
    restrict: 'A',
    require: '?^uibdropdowndummy',
    link: function(scope, element, attrs, dropdowndummyCtrl) {
...
angular.module("uib/template/datepicker/day.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("uib/template/datepicker/day.html",
    "<table class=\"uib-daypicker date_table\" role=\"grid\" aria-labelledby=\"{{::uniqueId}}-title\" aria-activedescendant=\"{{activeDateId}}\">\n" +
    "  <thead>\n" +
    "    <tr>\n" +
    "      <th><button type=\"button\" class=\"btn btn-default btn-sm pull-left uib-left\" ng-click=\"move(-1)\" tabindex=\"-1\"><i class=\"fa fa-caret-left\"></i></button></th>\n" +
    "      <th colspan=\"{{::5 + showWeeks}}\"><button id=\"{{::uniqueId}}-title\" role=\"heading\" aria-live=\"assertive\" aria-atomic=\"true\" type=\"button\" class=\"btn btn-default btn-sm uib-title\" ng-click=\"toggleMode()\" ng-disabled=\"datepickerMode === maxMode\" tabindex=\"-1\" style=\"width:100%;\"><strong>{{title}}</strong></button></th>\n" +
    "      <th><button type=\"button\" class=\"btn btn-default btn-sm pull-right uib-right\" ng-click=\"move(1)\" tabindex=\"-1\"><i class=\"fa fa-caret-right\"></i></button></th>\n" +
    "    </tr>\n" +
    "    <tr>\n" +
...
angular.module("uib/template/datepicker/month.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("uib/template/datepicker/month.html",
    ...
    "          ng-click=\"select(dt.date)\"\n" +
    "          ng-disabled=\"::dt.disabled\"\n" +
    "          tabindex=\"-1\"><span ng-class=\"::{'text-info_cstfont': dt.current}\">{{::dt.label}}</span></button>\n" +
    "      </td>\n" +
    "    </tr>\n" +
    "  </tbody>\n" +
    "</table>\n" +
...
angular.module("uib/template/datepicker/popup.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("uib/template/datepicker/popup.html",
    "<ul class=\"uib-datepicker-popup dropdown-menu cst_datepicker_grid\" dropdown-nested ng-if=\"isOpen\" style=\"display: block\" ng-style=\"{top: position.top+'px', left: position.left+'px'}\" ng-keydown=\"keydown($event)\" ng-click=\"$event.stopPropagation()\">\n" +
    "	<li ng-transclude></li>\n" +
